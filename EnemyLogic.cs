...
public class EnemyLogic : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (health <= 0)
        {
            ...
            {
                loc = new Vector3(transform.position.x, (float)(transform.position.y + 0.5), transform.position.z);
                if (Random.Range(1, 100) > 50)
                    Instantiate(rifleAmmo, loc, transform.rotation);
                else
                    Instantiate(pistolAmmo, loc, transform.rotation);
            ...
            {
                mission.curr += 1;
                mission.missionText.SetText("Eliminate the soldiers that are attacking the village!(" + mission.curr + " / 16)");
                if (mission.curr == 16)
                    mission.nextM();
            }
            else if (mission.index == 5)
            {
                mission.curr += 1;
                if (mission.curr == 8)
                {
                    mission.nextM();
                    GameObject.Find("Trigger").GetComponent<Trigger>().Disable();
                }
            }
        }
    }
    ...
    public void Patrolling()
    {
        ...
        if(!setTarget)
        {
            ...
            {
                if(min > Vector3.Distance(t.position, transform.position))
                {
                    min = Vector3.Distance(t.position, transform.position);
        ...
        if (aStarTimer == 2)
        {
            if(Vector3.Distance(pointList[pointIndex].position, transform.position) < 12.5)
            {
                patrolTimer = 0;
                pointIndex++;
            ...
            aStarPath = SearchAStar(transform, pointList[pointIndex]);
        }
        GoTo(aStarPath);
    ...
    public void Shoot()
    {
        ...
        {
            GameObject tempBullet = Instantiate(bullet, spawnBullet.transform.position, fireRotation);
